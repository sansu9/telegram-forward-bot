...
## 支持本项目
赞助商：三俗数字商城 https://sansu6.com 
| 专注解决苹果ID、ins、谷歌、电报、推特、静态住宅IP等海外社媒问题，提供一站式服务！
<!-- 换成你自己的文案和链接 -->
...
同一段文字内容如果在多个源群里重复出现（比如同一条通知被转发到好几个群），只会被转发一次。原理是给每条消息的文字内容算一个指纹，记录在本地的 `seen_messages.json` 里，`config.py` 里的 `DEDUP_WINDOW_DAYS`（默认 30 天）决定这个记录保留多久，超过这个天数的旧记录会被自动清掉，避免文件一直变大。

只对文字内容生效——没有文字说明的纯图片/视频消息没有内容可以用来去重，会照常转发。`seen_messages.json` 是运行时生成的数据，已经加进 `.gitignore`，不需要也不应该提交到仓库。
